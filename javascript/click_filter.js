const filterCountries = (target) => {
    ...
    let bplotCountry = bplot.select(`#${classList[2]}`)
    ...
    if (!bplotCountry.empty()) {
        bplotCountry.classed('selected', !bplotCountry.classed('selected')).style('fill', 'gray');
        bplot.selectAll(`rect:not(.selected)`).style('fill', color[9]);
    }
